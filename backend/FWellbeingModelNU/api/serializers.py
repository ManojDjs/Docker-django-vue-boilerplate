...
class TotalAnswers_A(serializers.ModelSerializer):
    FWBMNU=Answer_Eoboration(many=True,read_only=True)
    total_questions=serializers.SerializerMethodField()
    total_answers=serializers.SerializerMethodField()

    class Meta:
        model=FWBMNU
        fields=('id','Answered_by','FWBMNU','total_answers','total_questions')
    def get_total_answers(self,instance):
        ...
        u=[u for u in FWBMNU_answers.objects.filter(answer_by=user)]
        return len(u)

    def get_total_questions(self, instance):

        u = [u for u in FWBMNU_Questions.objects.all()]
        return len(u)
