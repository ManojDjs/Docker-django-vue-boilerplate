...
class TotalAnswers_A(serializers.ModelSerializer):
    WBMNU=Answer_Eoboration(many=True,read_only=True)
    total_questions=serializers.SerializerMethodField()
    total_answers=serializers.SerializerMethodField()

    class Meta:
        model=WBMNU
        fields=('id','Answered_by','WBMNU','total_answers','total_questions')
    def get_total_answers(self,instance):
        ...
        u=[u for u in WBMNU_answers.objects.filter(answer_by=user)]
        return len(u)

    def get_total_questions(self, instance):

        u = [u for u in WBMNU_Questions.objects.all()]
        return len(u)
