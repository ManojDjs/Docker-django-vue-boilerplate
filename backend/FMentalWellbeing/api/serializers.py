...
class TotalAnswers_A(serializers.ModelSerializer):
    FMWB=Answer_Eoboration(many=True,read_only=True)
    total_questions=serializers.SerializerMethodField()
    total_answers=serializers.SerializerMethodField()

    class Meta:
        model=FMWB
        fields=('id','Answered_by','FMWB','total_answers','total_questions')
    def get_total_answers(self,instance):
        ...
        u=[u for u in FMWB_answers.objects.filter(answer_by=user)]
        return len(u)

    def get_total_questions(self, instance):

        u = [u for u in FMWB_Questions.objects.all()]
        return len(u)
